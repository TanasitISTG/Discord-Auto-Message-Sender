...
fn main() {
    ...
    let sidecar_path = manifest_dir.join("resources").join("sidecar").join(sidecar_name);
    ...
    if profile != "release" && !sidecar_path.exists() {
        ...
        if let Some(bundle) = config.get_mut("bundle").and_then(|value| value.as_object_mut()) {
            bundle.insert("resources".to_string(), serde_json::Value::Array(Vec::new()));
        }

        let generated_config = serde_json::to_string(&config)
            .expect("Failed to serialize debug/test Tauri config.");
