            ...
            {showAdvancedTools ? (
                <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    onChange={(event: ChangeEvent<HTMLTextAreaElement>) =>
                                        draft.setImportDraft(event.target.value)
                                    }
                                />
                            </div>
                        </div>
                            ...
                            <div className="space-y-3 text-sm">
                                {!draft.state.importPreview ? (
                                    ...
                                    <>
                                        <StateRow
                                            label="Channels"
                                            value={String(draft.state.importPreview.channels.length)}
                                        />
                                        <StateRow
                                            label="Message groups"
                                            value={String(Object.keys(draft.state.importPreview.messageGroups).length)}
                                        />
                                        <StateRow
                                            label="Messages"
                                            value={String(
                                                Object.values(draft.state.importPreview.messageGroups).reduce(
                                        ...
                                        <StateRow
                                            ...
                                            value={
                                                draft.importPreviewErrors.length === 0
                                                    ? 'ready'
                                                    : `${draft.importPreviewErrors.length} issue(s)`
                                            }
                                        />
                                        {draft.importPreviewErrors.length > 0 ? (
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        <Button
                            variant="secondary"
                            onClick={() => draft.previewImport()}
                            disabled={!draft.state.importDraft.trim()}
                        >
                            Preview import
                        </Button>
                        <Button
                            onClick={() => draft.applyImport()}
                            disabled={!draft.state.importPreview || draft.importPreviewErrors.length > 0}
                        >
