    ...
    return (
        <Card className="flex flex-col xl:max-h-[calc(100vh-220px)]">
            <CardHeader>
                ...
                <CardDescription>
                    Keep group editing in a bounded side panel so it does not dictate the entire page height.
                </CardDescription>
            </CardHeader>
            <CardContent className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
                <div className="flex gap-2">
                    <Input
                        ...
                        value={draft.state.newGroupName}
                        onChange={(event: ChangeEvent<HTMLInputElement>) => draft.setNewGroupName(event.target.value)}
                    />
                        ...
                        <div className="grid max-h-44 gap-2 overflow-auto pr-1">
                            {groupEntries.map(([groupName, messages]) => (
                                ...
                                >
                                    ...
                                    <div className="text-[11px] font-semibold tracking-tight text-muted-foreground mt-0.5">
                            ...
                            <Field label="Selected group name">
                                <Input
                                    ...
                                    onChange={(event: ChangeEvent<HTMLInputElement>) =>
                                        draft.renameGroup(event.target.value)
                                    }
                                />
                            </Field>
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    value={draft.state.cloneGroupName}
                                    onChange={(event: ChangeEvent<HTMLInputElement>) =>
                                        draft.setCloneGroupName(event.target.value)
                                    }
                                />
                                    ...
                                    <div className="mb-2 flex items-center justify-between gap-2">
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                variant="ghost"
                                                onClick={() =>
                                                    draft.moveMessage(draft.state.selectedGroupName, index, -1)
                                                }
                                                disabled={index === 0}
                                            >
                                            ...
                                            <Button
                                                ...
                                                disabled={index === draft.selectedGroupMessages.length - 1}
                                            >
                                            ...
                                            <Button
                                                variant="ghost"
                                                onClick={() =>
                                                    draft.removeMessage(draft.state.selectedGroupName, index)
                                                }
                                            >
                ...
                <Button
                    className="w-full"
                    variant="secondary"
                    onClick={() => draft.addMessage(draft.state.selectedGroupName)}
