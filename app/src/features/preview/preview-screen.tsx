        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        No messages are sent. This is a local preview of channel selection, group resolution, and
                        cadence.
                    </CardDescription>
                </CardHeader>
                            ...
                            {dryRun.channels.map((channel) => (
                                ...
                                >
                                    ...
                                    <div className="mb-4 text-xs leading-relaxed text-muted-foreground">
                                        Cadence:{' '}
                                        {channel.cadence.numMessages === 0
                                            ? 'infinite until stopped'
                                            : `${channel.cadence.numMessages} messages`}{' '}
                                        with {channel.cadence.baseWaitSeconds}s base wait and{' '}
                                        {channel.cadence.marginSeconds}s margin.
                                    </div>
                                    <div className="space-y-2">
                                        {channel.sampleMessages.length === 0 ? (
                                            <div className="text-sm text-muted-foreground">
                                                No messages resolved for this channel.
                                            </div>
                                        ) : (
                                    ...
                                    {channel.skipReasons.length > 0 ? (
                                        <div className="mt-4 space-y-1 text-xs text-amber-300">
                                            {channel.skipReasons.map((reason) => (
